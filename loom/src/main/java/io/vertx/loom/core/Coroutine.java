...
/**
 * await() must be run on a Virtual Thread. Whenever we block, the Virtual
 * Thread will yield execution. So await() blocks until the Future.onComplete
 * handler unblocks us.
 */
class Coroutine {
	...
	public <A> A await(Single<A> single) {
		lock.lock();
		try {
